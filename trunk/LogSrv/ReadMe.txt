...
LogSrv.h
    ...
    헤더(Resource.h 포함)가 들어 있고 CLogSrvApp 응용 프로그램
    클래스를 선언합니다.

LogSrv.cpp
    응용 프로그램 클래스 CLogSrvApp이(가) 들어 있는 기본 응용 프로그램
    소스 파일입니다.
...
LogSrvDoc.h, LogSrvDoc.cpp - 문서
    이 파일에는 CLogSrvDoc 클래스가 들어 있습니다.  이 파일을 편집하여
    특수한 문서 데이터를 추가하고 CLogSrvDoc::Serialize를 통해 파일
    저장 및 로드를 구현합니다.

LogSrvView.h, LogSrvView.cpp - 문서 뷰
    이 파일에는 CLogSrvView 클래스가 들어 있습니다.
    CLogSrvView 개체는 CLogSrvDoc 개체를 보는 데 사용됩니다.
...
StdAfx.h, StdAfx.cpp
    이 파일은 미리 컴파일된 헤더(PCH) 파일인 LogSrv.pch와
